...
class CategoricalDDPNAS:
    ...
    def init_record(self):
        ...
        self.val_performance.append(
            np.zeros([self.p_model.d, self.p_model.Cmax]))
        ...
        if len(self.sample_index[0]) == 0:
            ...
            # when the total search is down
                ...
                for i in range(self.steps):
                    ...
                    expectation += softmax(
                        self.val_performance[i] * 100, axis=1)
                ...
                self.p_model.theta /= np.sum(self.p_model.theta,
                                             axis=1)[:, np.newaxis]
