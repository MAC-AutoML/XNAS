...
from xnas.algorithms.SNG.categorical import Categorical
...
class SNG:
    ...
    refer to: https://www.zhihu.com/question/21923317 for NG
    """
    ...
    def sampling(self):
        ...
        """
        rand = np.random.rand(
            self.p_model.d, 1)  # range of random number is [0, 1)
    ...
    def update_function(self, c_one, fxc, range_restriction=True):
        ...
        for i, K in enumerate(self.p_model.C):
            ...
            s_i += np.sqrt(theta_i) * \
                ng[i, :K - 1].sum() / (theta_K + np.sqrt(theta_K))
            sl += list(s_i)
        sl = np.array(sl)
    ...
    def update(self):
        if len(self.sample_index[0]) == 0:
            ...
            if self.pruning and self.current_step > self.steps:
                # pruning the index
                pruned_weight = deepcopy(self.p_model.theta)
