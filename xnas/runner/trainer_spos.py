...
class OneShotTrainer(Trainer):
    def __init__(self, supernet, criterion, optimizer, lr_scheduler, train_loader, test_loader, sample_type='iter'):
        super().__init__(
            model=supernet, 
            criterion=criterion, 
            optimizer=optimizer, 
    ...
    def train_epoch(self, cur_epoch, sample=None):
        ...
        self.train_meter.iter_tic()
        # cnt = 0
        # cycle = (len(self.train_loader)//8) 
        for cur_iter, (inputs, labels) in enumerate(self.train_loader):
            losses = 0
            choices = np.array([np.random.permutation(8) for _ in range(28)])
            inputs, labels = inputs.cuda(), labels.cuda(non_blocking=True)
            ...
            if isinstance(sample, list):
                ...
                for sample_ in sample:
                    preds = self.model(inputs, sample_)
                    loss = self.criterion(preds, labels)
                    loss.backward()
