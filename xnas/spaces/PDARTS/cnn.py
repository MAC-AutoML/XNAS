    ...
    def __init__(self, n_nodes, C_pp, C_p, C, reduction_p, reduction, basic_op_list, dropout_p):
        ...
        for i in range(self.n_nodes):
            ...
            for j in range(2+i):  # include 2 input nodes
                ...
                edg_id = pre+j
                op = _MixOp_PDarts(C, C, stride, self.p, self.basic_op_list[edg_id])
...
class PDartsCNN(nn.Module):
    def __init__(self, C_in=3, C=16, n_classes=10, n_layers=8, n_nodes=4, basic_op_list=[], p=0.0):
        super().__init__()
        self.p = float(p)
        stem_multiplier = 3
        self.C_in = C_in  # 3
...
def _PDartsCNN(**kwargs):
    from xnas.core.config import cfg
    return PDartsCNN(
        C_in=cfg.SEARCH.INPUT_CHANNELS,
        C=cfg.SPACE.CHANNELS+kwargs['add_width'],
        n_classes=cfg.LOADER.NUM_CLASSES,
        n_layers=cfg.SPACE.LAYERS+kwargs['add_layers'],
        n_nodes=cfg.SPACE.NODES,
        basic_op_list=kwargs['basic_op'],
        p=float(kwargs['dropout_rate']))