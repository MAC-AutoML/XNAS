...
class _MixedOp_PCDarts(nn.Module):
    def __init__(self, C_in, C_out, stride, basic_op_list=None):
        ...
        for primitive in basic_primitives:
            op = OPS[primitive](C_in//self.k, C_out//self.k, stride, affine=False)
            self._ops.append(op)
    ...
    def __init__(self, n_nodes, C_pp, C_p, C, reduction_p, reduction, basic_op_list, multiplier):
        ...
        for i in range(self.n_nodes):
            ...
            for j in range(2+i):  # include 2 input nodes
                ...
                op = _MixedOp_PCDarts(C, C, stride, self.basic_op_list)
...
class PCDartsCNN(nn.Module):
    def __init__(self, C_in=3, C=16, n_classes=10, n_layers=8, n_nodes=4, basic_op_list=[], multiplier=4):
        super().__init__()
        stem_multiplier = 3
        self._multiplier = multiplier
        self.C_in = C_in  # 3
...
def pc_parse_from_numpy(alpha, k, basic_op_list=None):
    ...
    for edges in alpha:
        ...
        edge_max, primitive_indices = torch.topk(
            (edges[:, 1:]).clone().detach(), 1)  # ignore 'none'
