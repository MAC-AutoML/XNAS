...
class OFATrainer(KDTrainer):
    ...
    def validate_all_subnets(
        ...
        if ks_list is None:
            ks_list = copy.deepcopy(tmp_model.ks_list)
        if expand_ratio_list is None:
            expand_ratio_list = copy.deepcopy(tmp_model.expand_ratio_list)
        if depth_list is None:
            depth_list = copy.deepcopy(tmp_model.depth_list)
        if width_mult_list is None:
            if "width_mult_list" in tmp_model.__dict__:
                width_mult_list = copy.deepcopy(list(range(len(tmp_model.width_mult_list))))
            else:
                width_mult_list = [0]
        del tmp_model
